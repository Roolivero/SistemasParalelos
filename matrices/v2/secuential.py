"""Multiplicacion C = A*B en forma secuencial; verificacion con B^T A^T = (AB)^T."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Matmul secuencial (metodo transpuesto)")
    ...
    print_implementation_title("secuencial")

    print("[Progreso] Iniciando fase A·B...", flush=True)
    t0 = perf_counter()
    c_ab = matmul_transpose_method(a, b, dot_sequential)
    t1 = perf_counter()
    ...
    t2 = perf_counter()
    c_bt_at = matmul_transpose_method(bt, at, dot_sequential)
    t3 = perf_counter()
